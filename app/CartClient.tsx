...
export default function CartClient({ initialCartData }: { initialCartData: CartData }) {
  ...
  return (
    // pb-24 so content doesn't hide behind sticky footer on mobile
    <div className="pb-24 lg:pb-0">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
            ...
            {cart.cartItems.length === 0 ? (
              ...
              <div className="divide-y divide-gray-50">
                {cart.cartItems.map((item) => (
                  <div key={item.product_id} className="p-4 sm:p-5 flex gap-3 sm:gap-4">
                    <img src={item.image} alt={item.product_name}
                      className="w-16 h-16 sm:w-20 sm:h-20 rounded-lg object-cover border border-gray-100 flex-shrink-0" />
                    <div className="flex-1 min-w-0">
                      <p className="font-semibold text-gray-800 text-sm leading-snug">{item.product_name}</p>
                      <p className="text-green-700 font-bold mt-1 text-sm">₹{item.product_price}</p>
                      <div className="flex items-center justify-between mt-3">
                        ...
                        <div className="text-right">
                          <p className="font-bold text-gray-800">₹{item.product_price * item.quantity}</p>
        ...
        {/* Order summary — desktop only, on mobile the sticky bar handles CTA */}
        <div className="lg:col-span-1">
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5 lg:sticky lg:top-20">
            ...
            <button onClick={() => router.push("/checkout")}
              ...
              className="mt-4 w-full bg-green-600 hover:bg-green-700 disabled:bg-gray-300 disabled:cursor-not-allowed text-white font-bold py-3 rounded-lg text-sm transition-colors hidden lg:block">
              Proceed to Checkout →
            </button>

            <p className="text-center text-xs text-gray-400 mt-3 hidden lg:block">🔒 Safe & Secure Payments</p>
          </div>
        </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 px-4 py-3 z-50 lg:hidden">
        <div className="flex flex-col gap-1 mb-1">
          <div className="flex justify-between text-xs text-gray-500">
            <span>Total ({cart.cartItems.length} items)</span>
            <span className="font-bold text-gray-800">₹{total}</span>
          </div>
        </div>
        <button onClick={() => router.push("/checkout")}
          disabled={cart.cartItems.length === 0}
          className="w-full bg-green-600 hover:bg-green-700 disabled:bg-gray-300 disabled:cursor-not-allowed text-white font-bold py-3 rounded-lg text-sm transition-colors">
          Proceed to Checkout →
        </button>
      </div>
    </div>
