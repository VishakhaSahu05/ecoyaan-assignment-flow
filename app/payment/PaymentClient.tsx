...
export default function PaymentClient() {
  ...
  const { cartData, shippingAddress, subtotal, grandTotal } = useCart();
  const [method, setMethod] = useState<PaymentMethod>("upi");
  const [upiId, setUpiId] = useState("");
  const [upiError, setUpiError] = useState("");
  const [loading, setLoading] = useState(false);

  // if someone directly opens /payment without going through the flow
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
      ...
      <div className="lg:col-span-2 space-y-4">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100">
          ...
          {/* Card form - UI only, not wired to real payment */}
          {method === "card" && (
        ...
        <div className="flex gap-3">
          ...
          <button
            onClick={handlePay}
            disabled={loading}
            className="flex-1 bg-green-600 hover:bg-green-700 disabled:bg-green-400 text-white font-bold py-2.5 rounded-lg text-sm transition-colors flex items-center justify-center gap-2"
          >
            {loading ? (
              <>
